...
const activeTab = ref<'messages' | 'activities' | 'documents' | 'notifications'>('messages')
...
<template>
  <aside class="panel scroll-thin hidden h-[calc(100vh-6.5rem)] w-[360px] min-w-[360px] flex-col overflow-y-auto p-3 2xl:flex">
    <template v-if="props.task">
      ...
      <div class="mt-4 grid grid-cols-2 gap-2 text-xs">
        <button class="panel-muted px-2 py-1.5" @click="activeTab = 'messages'">Messages ({{ props.messages.length }})</button>
        <button class="panel-muted px-2 py-1.5" @click="activeTab = 'activities'">Activities ({{ props.activities.length }})</button>
        <button class="panel-muted px-2 py-1.5" @click="activeTab = 'documents'">Documents ({{ props.documents.length }})</button>
        <button class="panel-muted px-2 py-1.5" @click="activeTab = 'notifications'">Notifications ({{ props.notifications.length }})</button>
      </div>

      <div class="mt-3 space-y-2">
        <template v-if="activeTab === 'messages'">
          <article v-for="message in props.messages" :key="message.id" class="panel-muted p-2">
        ...
        <template v-else-if="activeTab === 'activities'">
          <article v-for="activity in props.activities" :key="activity.id" class="panel-muted p-2">
        ...
        <template v-else-if="activeTab === 'documents'">
          <article v-for="document in props.documents" :key="document.id" class="panel-muted p-2">
            <p class="text-sm font-semibold">{{ document.title }}</p>
            <p class="text-xs uppercase tracking-wide text-[rgb(var(--muted-foreground))]">{{ document.type }}</p>
            <pre class="mt-2 overflow-x-auto whitespace-pre-wrap text-xs text-[rgb(var(--muted-foreground))]">{{ document.content }}</pre>
          </article>
          <p v-if="props.documents.length === 0" class="text-sm text-[rgb(var(--muted-foreground))]">Sem documentos.</p>
        </template>

        <template v-else>
          <article v-for="notification in props.notifications" :key="notification.id" class="panel-muted p-2">
    ...
    <template v-else>
      <div class="panel-muted flex h-full items-center justify-center p-4 text-center text-sm text-[rgb(var(--muted-foreground))]">
        Selecione uma task no board para ver mensagens, atividades, documentos e notificações.
      </div>
    </template>
  </aside>
</template>
