...
export default defineEventHandler(async (event) => {
  ...
  const existingUser = await convexQuery<any>('users:findByEmail', { email: normalizedEmail })
  if (existingUser) {
    throw createError({ statusCode: 409, statusMessage: 'Já existe um usuário com este email.' })
  }
