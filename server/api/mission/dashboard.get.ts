...
type DashboardPayload = {
  ...
  messagesByTask: Record<string, Array<{ id: string; taskId: string; fromAgentId: string; content: string; attachments: string[]; timestampLabel: string }>>
  activitiesByTask: Record<string, Array<{ id: string; taskId: string; type: 'task_created' | 'message_sent' | 'document_created' | 'status_changed'; agentId: string; message: string; timestampLabel: string }>>
  documentsByTask: Record<string, Array<{ id: string; taskId: string; title: string; type: 'deliverable' | 'research' | 'protocol'; content: string }>>
  notificationsByTask: Record<string, Array<{ id: string; taskId: string; mentionedAgentId: string; content: string; delivered: boolean }>>
}
...
function normalizeTaskStatus(value: string): TaskStatus {
  ...
  if (value === 'inbox' || value === 'assigned' || value === 'in_progress' || value === 'review' || value === 'done') {
    return value
  }
...
export default defineEventHandler(async (event): Promise<DashboardPayload> => {
  ...
  const [rawAgents, rawTasks, rawMessages, rawActivities, rawDocuments, rawNotifications] = await Promise.all([
  ...
  const agents = rawAgents.map((agent) => ({
    ...
    status: agent.status === 'active' || agent.status === 'blocked' ? agent.status : 'idle',
  ...
  const tasks = rawTasks.map((task) => ({
    ...
    assigneeIds: Array.isArray(task.assigneeIds) ? task.assigneeIds.map((id: unknown) => String(id)) : [],
    labels: Array.isArray(task.labels) ? task.labels.map((label: unknown) => String(label)) : [],
  }))
  ...
  const messages = rawMessages.map((message) => ({
    ...
    attachments: Array.isArray(message.attachments) ? message.attachments.map((id: unknown) => String(id)) : [],
  ...
  const activities = rawActivities.map((activity) => ({
    ...
    type:
      activity.type === 'task_created' || activity.type === 'message_sent' || activity.type === 'document_created'
        ? activity.type
        : 'status_changed',
  ...
  const documents = rawDocuments.map((document) => ({
    ...
    type: document.type === 'research' || document.type === 'protocol' ? document.type : 'deliverable',
